...
export const t8Resources: Resource[] = [
  ...
  // --------------------------------------------------------------------------
  {
    ...
    whyForHim:
      "The spec's #1 key finding is that his ROI is in demonstrable artifacts and a public reputation, not exams: a senior cyber generalist without a developer title gets hired as FDE/founding SE on visible proof, and the five-year plan says he needs a visible reputation ('FOMO') before founding. Learning in public turns each track into a repo plus a post, feeds the weekly d-tech-post-week drill and the Phase 1 threshold ('one public repo + one post live'), and lets him tell the failed-founding story on his own terms — the plan's 'reframe the failed attempt as a lesson learned' positioning.",
    priority: "must_do",
    ...
    freshness: "current",
    qualitySignal:
      "Spec: 'Must-adopt'. Key Finding #1 — 'he should learn by building in public, not by collecting credentials' — anchors every track to portfolio outputs. swyx's 2018 essay (verified live 2026-09-16) is the canonical statement of the idea: create 'learning exhaust' by sharing what you learn as you learn it.",
    notes:
  ...
  // --------------------------------------------------------------------------
  {
    ...
    notes:
      "Keep long-form thinking in the vault and short linked notes in Forge's Notes view (both are markdown, so either exports to the other). Alternative if he prefers databases/shared pages: t8-notion. Anki (t8-anki) handles recall; Obsidian handles synthesis. Pick one PKM and stop — the spec's Track 8 rule is 'curate lightweight; don't over-invest'.",
  },
  ...
  // --------------------------------------------------------------------------
  {
    ...
    whyForHim:
      "The spec offers Notion as the alternative to Obsidian. It earns its place only if he wants databases/kanban and pages he can share — e.g., a co-founder trial-project workspace (the Phase 3 milestone), design-partner interview notes from d-customer-interview-week, or a PRD he co-edits with a founder once he is inside a seed-stage startup. Otherwise Obsidian (free, local) is the default.",
    priority: "optional",
    producesArtifact: false,
    trackIds: ["T8_META"],
    skillIds: ["pkm"],
  ...
  // --------------------------------------------------------------------------
  {
    ...
    whyForHim:
      "The System Design Primer (T2 must-do, 368k GitHub stars) ships its own Anki decks and the spec explicitly pairs Anki with them, so the vocabulary sticks between weekly d-sysdesign-week drills. T2 is his CTO-credibility track and the T1/T2 self-tests ('explain attention, tokenization, KV cache, DPO vs RLHF to a peer'; '5 canonical system-design questions') are recall tests — recall is what fails under interview and customer pressure.",
    priority: "high",
    producesArtifact: false,
  ...
  // --------------------------------------------------------------------------
  // t8-israeli-meetups — his geography as an accountability + discovery loop
